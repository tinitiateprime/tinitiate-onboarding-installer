...
## Step 1: run the basic installer (Python + Java)

Run the installer for your computer once. It installs the following software and the configured Python libraries and VS Code extensions automatically. You do not need to install Python, Java, Node.js, Git, Chocolatey, or Homebrew first; the installer sets them up.
...
Wait for **installation is complete**, then restart Windows and open Docker Desktop.
...
When installation finishes, open Docker Desktop.
...
For Python, run `python --version` on Windows or `python3 --version` on macOS. Each command should print a version number. If a command fails, restart your terminal and try again. If it still fails, rerun the installer.

<details>
<summary>Check Python libraries and VS Code extensions</summary>
...
| [On-premises databases](environments/onprem-db/README.md) | `environments/onprem-db/compose.yaml` | SQL Server, PostgreSQL with pgvector, MySQL; optional Oracle and DynamoDB Local | Not needed |
...
Complete the basic installation first. [Download this project](https://github.com/tinitiateprime/tinitiate-onboarding-installer/archive/refs/heads/main.zip) and extract the ZIP. Open **PowerShell on Windows** or **Terminal on macOS**, type `cd `, drag the extracted folder into the window, and press **Enter**. Run the course commands below from that folder.
...
This starts SQL Server, PostgreSQL, and MySQL. For Oracle or DynamoDB Local, use the commands in the [on-premises database guide](environments/onprem-db/README.md#run-only-selected-databases).
